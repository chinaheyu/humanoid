...
static bool verify_crc8(uint8_t *pchMessage, size_t dwLength)
{
    ...
    ucExpected = get_crc8(pchMessage, dwLength - 1);
...
static void append_crc8(uint8_t *pchMessage, size_t dwLength)
{
    ...
    ucCRC = get_crc8((uint8_t *)pchMessage, dwLength - 1);
...
static bool verify_crc16(uint8_t *pchMessage, size_t dwLength)
{
    ...
    wExpected = get_crc16(pchMessage, dwLength - 2);
...
static void append_crc16(uint8_t *pchMessage, size_t dwLength)
{
    ...
    wCRC = get_crc16((uint8_t *)pchMessage, dwLength - 2);
...
static uint32_t get_crc32(uint8_t *pchMessage, size_t dwLength)
{
    ...
    {
        chData = *pchMessage++;
        (wCRC) = ((uint32_t)(wCRC) >> 8)  ^ CRC32_TAB[((uint32_t)(wCRC) ^ (uint32_t)(chData)) & 0x000000ff];
    }
...
static bool verify_crc32(uint8_t *pchMessage, size_t dwLength)
{
    ...
    wExpected = get_crc32(pchMessage, dwLength - 4);
...
static void append_crc32(uint8_t *pchMessage, size_t dwLength)
{
    ...
    wCRC = get_crc32((uint8_t *)pchMessage, dwLength - 4);
...
bool protocol_unpack_byte(protocol_stream_t* unpack_stream, uint8_t byte)
{
    switch (unpack_stream->unpack_step)
    {
        ...
        case STEP_LENGTH_HIGH:
        {
            ...
            {
                unpack_stream->unpack_step = STEP_HEADER_CRC8;
            }
            else
            {
